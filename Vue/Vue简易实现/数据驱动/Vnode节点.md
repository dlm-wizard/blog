...
```bash
# VNode 是对真实 DOM 的一种抽象描述它的
核心：tag、data、children、key [其他属性来扩展 VNode、实现特殊 feature]

1. 对真实 DOM 的一层抽象 [不依赖于平台]
2. 用一个简单的原生 JS 对象去模拟一个 DOM 节点  [轻量和简单]
...
2. tag不存在的时候同样创建一个空节点。当tag不是一个String类型的时候代表tag是一个组件的构造类，直接用new VNode创建。
3. 当tag是String类型的时候：
    * 如果是保留标签，则用new VNode创建一个VNode实例
...
export function createElement(
  ...
  /*手写render函数: ALWAYS_NORMALIZE*/
  /*模板编译: SIMPLE_NORMALIZE*/
...
```bash
1. string 类型，则接着判断
     * 如果是html原生标签，则直接创建一个普通 VNode
     * 如果是为已注册的组件名，则通过 createComponent 创建一个组件类型的 VNode，
     * 否则创建一个未知的标签的 VNode。 

2. tag 一个 Component 类型，则直接调用 createComponent 创建一个组件类型的 VNode 节点
```  
