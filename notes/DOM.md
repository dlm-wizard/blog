...
DOM1 标准制定出来后，浏览器不仅要按照新的标准去支持 HTML 和 CSS，还要对老旧的网页进行兼容，而这一前一后的两种支持方式，就是我们所称的`标准模式(StandardMode)`和`怪异模式(QuirksMode)`。对于现代规范的网页，浏览器一般用w3c的标准解析和渲染,而对于古老的网页,浏览器就会使用怪异模式去渲染，也就是以自己的方式向后兼容老版本。
