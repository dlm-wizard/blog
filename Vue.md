在过去的几年中，我们可以通过 JavaScript 更强大的实现和页面动态交互效果，我们将原来在服务端的大量代码移动到了浏览器端执行，但这样的行为留给了我们数以千计的 JavaScript 代码需要和 html 页面，与 css 进行关联使用，但是我们却并没有什么规范组织代码的方法，这也是大量的开发者使用 framework 进行开发的原因。
